...
const SlidingCards: React.FC<SlidingCardsProps> = ({
  ...
  useEffect(() => {
    ...
    const updatePositions = () => {
      cardsRef.current.forEach((card, i) => {
        ...
        card.style.transform = `perspective(700px) translateZ(${
          -12 * offset
        }px) translateY(${7 * offset}px) translateX(0px) rotateY(0deg)`;
    ...
    const handleEnd = () => {
      ...
      if (card) {
        ...
        if (Math.abs(deltaX) > threshold) {
          ...
          card.style.transform = `perspective(700px) translateZ(-12px) translateY(7px) translateX(${
            direction * 300
          }px) rotateY(${direction * 20}deg)`;
  ...
  return (
    <section
      ref={cardStackRef}
      className={cn(
        "relative w-[400px] h-[650px] grid place-content-center touch-none select-none",
        className
      )}
    >
      {cards.map(({ id, icon, bgClass = "" }, index) => (
        <article
          key={id}
          onClick={() => onCardClick?.(index)}
          className={cn(
            "card absolute inset-4 grid place-content-center cursor-grab transition-transform ease-in-out",
            bgClass
          )}
        >
          <span
            className={cn("aspect-square grid place-content-center", cardSize)}
          >
            {icon || (
              <svg
